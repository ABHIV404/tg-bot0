...
async def new_email(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            session.commit()
            Session.remove()
            await query.message.reply_text(f"Your new email: `{address}`", parse_mode='Markdown')
        else:
            await query.message.reply_text("Failed to create email. Please try again.")
    except Exception as e:
        logger.error(f"Email creation error: {e}")
        await query.message.reply_text("Email service unavailable. Try again later.")

async def check_inbox(update: Update, context: CallbackContext):
    query = update.callback_query
    await query.answer()
    await query.message.reply_text("Inbox feature coming soon!")

async def delete_email(update: Update, context: CallbackContext):
    query = update.callback_query
    await query.answer()
    await query.message.reply_text("Delete feature coming soon!")
